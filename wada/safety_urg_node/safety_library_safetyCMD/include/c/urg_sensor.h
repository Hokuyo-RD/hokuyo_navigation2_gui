...
#include "urg_connection.h"
    ...
    */
	...
	/*!
      \~japanese
      \brief 安全センサーの測定データの構造

      \~english
      \brief Safety sensor measument data structure
    */
    typedef struct
    {
        int is_setting;
		...
		int error_code;
		int is_lockout;
		int is_ossd1_on;        //OSSD 1_1
		int is_ossd2_on;        //OSSD 1_2
		int is_warning1_on;
		int is_warning2_on;
		int is_ossd3_on;        //OSSD 2_1
		int is_ossd4_on;        //OSSD 2_2
        int reserved1;
        int reserved2;
		int is_mut_over1_on;
		int is_mut_over2_on;
		...
		long timestamp;
		int is_laser_off;									//Compatible from version 2.0.0 onwards
		int is_optical_window_contamination_warning;		//Compatible from version 2.1.0 onwards
        int encoder_input_pattern_num;
        int encoder_angular_velocity;
        int protection_z1_detect_start_step;
        int protection_z1_detect_end_step;
        int protection_z2_detect_start_step;
        int protection_z2_detect_end_step;
        int warning_z1_detect_start_step;
        int warning_z1_detect_end_step;
        int warning_z2_detect_start_step;
        int warning_z2_detect_end_step;
    } safety_data_t;
	 ...
	 typedef struct
    {
		...
		int encoder_velocity;
		int is_laser_off;
		int is_slave1_ossd12_on;  //OSSD 1,2
		int is_slave2_ossd12_on;  //OSSD 1,2
		int is_slave3_ossd12_on;  //OSSD 1,2
		int is_slave1_ossd34_on;  //OSSD 3,4
		int is_slave2_ossd34_on;  //OSSD 3,4
		int is_slave3_ossd34_on;  //OSSD 3,4
		int is_slave1_warning1_on;
		int is_slave2_warning1_on;
		int is_slave3_warning1_on;
		int is_slave1_warning2_on;
		int is_slave2_warning2_on;
		int is_slave3_warning2_on;
		int is_slave1_error_detected;
		int is_slave2_error_detected;
		int is_slave3_error_detected;
		int is_slave1_laser_off;
		int is_slave2_laser_off;
		int is_slave3_laser_off;
		long timestamp;
		int is_optical_window_contamination_warning;
        int encoder_input_pattern_num;
        int encoder_angular_velocity;
		char reserved[34];
    } status_data_t;
	 ...
	 typedef union 
	 {
		 struct
		 {
			 unsigned area				: 8;
			 unsigned reserved			: 4;
			 unsigned protection1_state	: 1;
			 unsigned protection2_state	: 1;
		 };
		 int io_all;
	 } log_io_t;
	 ...
	 {
      log_io_t io;
      int protection1_min_dist;
      int protection1_min_dist_step;
      int protection2_min_dist;
      int protection2_min_dist_step;
      log_io_t slave1_io;
      char reserved1;
      char reserved2;
      log_io_t slave2_io;
    ...
    */
	...
	/*!
      ...
      \return sensor model string
    */
    extern const char *safety_sensor_product_type(urg_t *urg);
    ...
    /*!
      ...
      \return serial number string
    */
    extern const char *safety_sensor_serial_id(urg_t *urg);
    ...
    /*!
      ...
      \return firmware version string
    */
    extern const char *safety_sensor_firmware_version(urg_t *urg);
    ...
    */
    extern long urg_scip_decode(const char data[], int size);
	 ...
	 /*!
      ...
      \param[in,out] urg URG センサ管理
      \param[out] status_data　データ 

      \retval >=0 受信したデータ個数
      \retval <0 エラー

      data には、センサから取得したステーラスデータが格納されます。data はデータを格納するのサイズを確保しておく必要があります。data に格納されるデータ数は urg_max_data_size() で取得できます。
      ...
      Receives status data from the sensor. 
      ...
      Status data received from the sensor are stored in data array. data array should be previously allocated to hold all the data  requested from the sensor. To know how much data is received, use the urg_max_data_size() function.
    ...
    */
   	extern int safety_get_status(urg_t *urg, status_data_t *status_data);
    ...
    /*!
      ...
      \param[in,out] urg URG センサ管理
      \param[out] data エリアデータ [mm]
     
      \retval >=0 受信したデータ個数
      \retval <0 エラー

      data には、センサから取得したエリアデータが格納されます。data はデータを格納するのサイズを確保しておく必要があります。data に格納されるデータ数は urg_max_data_size() で取得できます。

      \~english
      \brief Gets area data

      Receives area data from the sensor.

      \param[in,out] urg URG control structure
      \param[out] data area data array [mm]
      
      \retval >=0 Number of data points received
      \retval <0 Error

      Area data received from the sensor are stored in data array. Data array should be previously allocated to hold all the data points requested from the sensor. To know how many data points are received, use the urg_max_data_size() function.
    ...
    */
	extern int safety_get_area(urg_t *urg, long data[]);
	 ...
	 /*!
      ...
      \param[in,out] urg URG センサ管理
      \param[out] detection_log_data　ログデータ 

      \retval >=0 受信したデータ個数
      \retval <0 エラー

      data には、センサから取得したログデータが格納されます。data はデータを格納するのサイズを確保しておく必要があります。data に格納されるデータ数は urg_max_data_size() で取得できます。

     

      \~english
      \brief Gets detection log data

      Receives log data from the sensor. 

      \param[in,out] urg URG control structure
      \param[out] detection_log_data detection log data
      
      \retval >=0 Number of data  received
      \retval <0 Error

      Log data received from the sensor are stored in data array. data array should be previously allocated to hold all the data  requested from the sensor. To know how much data is received, use the urg_max_data_size() function.
    ...
    */
   	extern int safety_get_detection_log(urg_t *urg, detection_log_data_t *detection_log_data);
	...
	/*!
     ...
     \~english
      \brief Clears detection log data

	  Clears the detection log data in the sensor
    ...
    */
   	extern int safety_get_detection_log_clear(urg_t *urg);
