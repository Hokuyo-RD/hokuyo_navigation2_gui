...
def read_csv(file_path):
    ...
    with open(file_path, mode='r') as csvfile:
        ...
        for row in reader:
            if len(row) == 10:  # ID, 位置（3）、サイズ、速度, 密度、向き（3）の3列があることを確認
                try:
                    data.append([int(row[0]), float(row[1]), float(row[2]),float(row[3]),int(row[4]),float(row[5]),float(row[6]),float(row[7]),float(row[8]),float(row[9])])
                except ValueError:
...
def publisher():
    ...
    while not rospy.is_shutdown():
        ...
        for _ in range(batch_size):
            # 現在のデータを取得
            id_value, position_x,position_y,position_z,size,velocity_value, density_value,pose_x,pose_y,pose_z = data[index]
            ...
            # パブリッシュ
            rospy.loginfo(f"Publishing: ID={msg.id},Position_x = {msg.position.x},Position_y = {msg.position.y},Position_z = {msg.position.z},Size = {msg.size}, Velocity={msg.velocity}, Density={msg.density},Pose_x = {msg.pose.x},Pose_y = {msg.pose.y},Pose_z = {msg.pose.z}")
            pub.publish(msg)
