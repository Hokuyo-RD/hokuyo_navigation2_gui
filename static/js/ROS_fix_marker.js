  ...
  otosimono_sub.subscribe(function (message) {
    ...
    for (var item of message.lostitem) {
      if ([1, 2, 3].includes(item.type)) {
        var lostType = item.type - 1;
        lost_prop_control.addMarker(lostType, new AdvancedMarker(L.marker([item.latitude, item.longitude], { icon: getLostIconFromId(lostType) }), item.id));
      }
    }
  })
